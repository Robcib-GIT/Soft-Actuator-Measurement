"""
A partir de: https://github.com/Infineon/RaspberryPi_DPS/tree/master
"""
...
class DPS:
    ...
    def __init__(self, adress: int, bus: int):
    ...
    def __correctTemperature(self):
        ...
        self.bus.write_byte_data(self.adress, 0x0E, 0xA5)

        self.bus.write_byte_data(self.adress, 0x0F, 0x96)

        self.bus.write_byte_data(self.adress, 0x62, 0x02)

        self.bus.write_byte_data(self.adress, 0x0E, 0x00)

        self.bus.write_byte_data(self.adress, 0x0F, 0x00)
    ...
    def __setOversamplingRate(self):
        ...
        self.bus.write_byte_data(self.adress, 0x06, 0x26)

        self.bus.write_byte_data(self.adress, 0x07, 0xA6)

        self.bus.write_byte_data(self.adress, 0x08, 0x07)

        # Oversampling Rate Configuration

        self.bus.write_byte_data(self.adress, 0x09, 0x0C)
    ...
    def __getRawPressure(self):
        ...
        p1 = self.bus.read_byte_data(self.adress, 0x00)

        p2 = self.bus.read_byte_data(self.adress, 0x01)

        p3 = self.bus.read_byte_data(self.adress, 0x02)
    ...
    def __getRawTemperature(self):
        ...
        t1 = self.bus.read_byte_data(self.adress, 0x03)

        t2 = self.bus.read_byte_data(self.adress, 0x04)

        t3 = self.bus.read_byte_data(self.adress, 0x05)
    ...
    def __getPressureCalibrationCoefficients(self):
        ...
        src13 = self.bus.read_byte_data(self.adress, 0x13)

        src14 = self.bus.read_byte_data(self.adress, 0x14)

        src15 = self.bus.read_byte_data(self.adress, 0x15)

        src16 = self.bus.read_byte_data(self.adress, 0x16)

        src17 = self.bus.read_byte_data(self.adress, 0x17)

        src18 = self.bus.read_byte_data(self.adress, 0x18)

        src19 = self.bus.read_byte_data(self.adress, 0x19)

        src1A = self.bus.read_byte_data(self.adress, 0x1A)

        src1B = self.bus.read_byte_data(self.adress, 0x1B)

        src1C = self.bus.read_byte_data(self.adress, 0x1C)

        src1D = self.bus.read_byte_data(self.adress, 0x1D)

        src1E = self.bus.read_byte_data(self.adress, 0x1E)

        src1F = self.bus.read_byte_data(self.adress, 0x1F)

        src20 = self.bus.read_byte_data(self.adress, 0x20)

        src21 = self.bus.read_byte_data(self.adress, 0x21)
    ...
    def __getTemperatureCalibrationCoefficients(self):
        ...
        src10 = self.bus.read_byte_data(self.adress, 0x10)

        src11 = self.bus.read_byte_data(self.adress, 0x11)

        src12 = self.bus.read_byte_data(self.adress, 0x12)
    ...
    def measurePressureOnce(self, units):
        ...
        # Validar la unidad
        if units not in ['Pa', 'mmHg']:
            raise ValueError("La unidad debe ser 'Pa' o 'mmHg'.")
        ...
        pressure =self.calcCompPressure(p, t) - self.tare_value

        
        if units == 'mmHg':
            # Convertir de Pa a mmHg
            return pressure / 133.322
        elif units == 'Pa':
            # Si ya está en Pa, devolver el valor tal cual
            return pressure
        
    def tare_pressure(self):
        self.tare_value = self.measurePressureOnce("Pa")

    def reset_tare_pressure(self):
        self.tare_value = 0
